...
def loglike(theta,data):
	...
	vehicle.set_torque(zero_torque)
	sigmas = np.array(theta[-(data.shape[0]):]).reshape(-1,1)
	#Need to update the parameters using the update params method
	vehicle.update_params(Cf=theta[0],Cr=theta[1],krof=theta[2],kror=theta[3],brof=theta[4],bror=theta[5],hrcf=theta[6],hrcr=theta[7],Jx=theta[8],Jz=theta[9])
	mod = solve_ivp(vehicle.model,t_span=[time[0],time[-1]],y0 = list(vehicle.states.values()),method = 'RK45',t_eval = time,rtol = 10e-4,atol = 10e-8)
	vehicle.reset_state()
...
def main():
	...
	with open('vd_14dof_exp4.npy', 'rb') as f:
		data = np.load(f)
	...
	with pm.Model() as model:
		...
		krof = pm.Uniform("krof",lower=10000,upper=70000,initval = 20000)
		kror = pm.Uniform("kror",lower=10000,upper=70000,initval = 20000)
		brof = pm.Uniform("brof",lower=100,upper=6000,initval = 2000)
		bror = pm.Uniform("bror",lower=100,upper=6000,initval = 2000)
		hrcf = pm.Uniform("hrcf",lower = 0.2, upper = 1.4, initval=0.7)
		hrcr = pm.Uniform("hrcr",lower = 0.2, upper = 1.4, initval=0.7)
		Jx = pm.Uniform("Jx",lower = 300,upper = 1500,initval=900)
		Jz = pm.Uniform('Jz',lower = 200, upper = 3500,initval=1500)
		sigmaRA = pm.HalfNormal("sigmaRA",sigma = 0.008,initval=0.008)
		sigmaRR = pm.HalfNormal("sigmaRR",sigma = 0.003,initval=0.003)
		sigmaLV = pm.HalfNormal("sigmaLV",sigma = 0.003,initval=0.003)
		...
		theta_ = [Cf,Cr,krof,kror,brof,bror,hrcf,hrcr,Jx,Jz,sigmaLV,sigmaRA,sigmaRR,sigmaYR]
